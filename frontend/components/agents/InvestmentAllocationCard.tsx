    ...
    <div className="space-y-3 text-sm text-(--ink-2)">
      ...
      <div className="rounded-xl bg-black/20 p-3">
        ...
        <div className="h-2 overflow-hidden rounded-full bg-white/10">
          <div className="h-full rounded-full bg-cyan-400/80" style={{ width: `${Math.max(0, Math.min(userCap, 100))}%` }} />
        </div>
      </div>

      <div className="rounded-xl bg-black/20 p-3 text-xs">
        <p className="text-(--muted)">Risk gate passed</p>
        <p className={data.risk_gate_passed ? "mt-1 font-semibold text-emerald-300" : "mt-1 font-semibold text-rose-300"}>
          {data.risk_gate_passed ? "True" : "False"}
        </p>
      </div>

      {data.reason && <p className="text-xs text-emerald-300">{data.reason}</p>}
    </div>
  );
}
